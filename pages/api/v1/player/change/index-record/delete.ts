...
export default async function handler(
  ...
  const results = await transaction()
    .query(
      SQL`DELETE FROM indexPlayerID 
            WHERE playerUpdateID=${req.body.indexRecord.playerUpdateID} 
            AND leagueID=${req.body.indexRecord.leagueID} 
            AND indexID=${req.body.indexRecord.indexID} 
            AND startSeason=${req.body.indexRecord.startSeason};`,
    )
    .rollback((_e: any) => {})
    .commit();
